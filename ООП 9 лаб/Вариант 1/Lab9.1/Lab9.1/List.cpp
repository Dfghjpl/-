...
void List::pop() {
	if (first == nullptr)
		throw exception("Список пустой. Нечего удалять");
...
void List::shift(int n) {
	if (n < 0) {
		throw exception("Индекс за пределами списка");
	}
	if (n == 0) {
		return;
	}
...
int& List::operator[](int index)
{
	...
	if (index >= size)
		throw exception("Индекс больше размера списка");
}
...
{
	if (L1.size != L2.size) {
		throw exception("Размеры списков не совпадают");
	}
	List result;
	Node* currA = L1.first;
	Node* currB = L2.first;
